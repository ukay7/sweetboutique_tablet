        ...
        <main className="store-shell app-surface">
            ...
            <Modal open={!!selected} onClose={() => setSelected(null)} className="product-modal">
                {selected && <div className="product-detail-layout">
                    ...
                    <div className="detail-copy"><span className="eyebrow">{selected.category?.name}</span><h2>{selected.name}</h2><p className="lead">{selected.description}</p><div className="detail-meta"><div><span>Starting from</span><b>{money(selected.base_price)}</b></div><div><span>Preparation</span><b>{selected.preparation_hours} {selected.preparation_hours === 1 ? 'hour' : 'hours'}</b></div></div>
